@foreach ($gurus as $cdt)
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-header">
            <h1 class="modal-title fs-5" id="modalDeleteLabel">Hapus Guru</h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
            <div class="modal-body">
                <form action="{{ url('guru/'. $id = $cdt -> id ) }}" method="post">
                    @method('DELETE')
                    @csrf
                    <div class="mb-3">
                        <p style="color: black">Apakah anda yakin untuk menghapus data {{ $cdt -> namaguru }} ?</p>
                    </div>
